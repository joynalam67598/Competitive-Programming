...
int main()
{
    ll   ttt,a,b, i, d, o, c, g, e, r, j, k, l, f = 0, h = 0, m,t=0, n, q, p, w, cnt;
    string s, s1;
    char cc;
    sf1(ttt);
    while (ttt--)
    {
        ...
        rep(i,0,n)
        {
            sf1(c),position.pb(c);uniq_pos.insert(c);
        ...
        if(uniq_pos.size()==1)
            cout<<min(position[n-1],b-position[n-1])<<endl;
        else{
            cnt=0;
            ll last_diff=0;
            ...
            rep(i,0,size_v-1)
            {
                ...
                {
                    cnt+= min3(position[i],b-position[i],abs(position[i]-position[i+1]));
                    last_diff=position[i];
                }
    //            debug(cnt);
            }
            if(position[size_v-1]!=last_diff)
                cnt+= min3(position[size_v-1],b-position[size_v-1],abs(position[size_v-1]-last_diff));
